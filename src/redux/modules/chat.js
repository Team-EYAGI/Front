...
/* ************ 새로 세팅 ************ */ 
// 채팅방 목록
const SET_NEW_CHAT_LIST = "SET_NEW_CHAT_LIST";
// 옮겨가는 (입장하려고 클릭한) 현재 방정보 입력
const MOVE_NEW_CHAT_ROOM = "MOVE_NEW_CHAT_ROOM";
// 뒤로가기 클릭시 현재방 정보 초기화
const NEW_CLEAR_CHAT = "NEW_CLEAR_CHAT";
// 구독하면서 실행되는 액션
// 새로입력되는 메세지(리스트 형태) 내용을 메세지에 추가
const NEW_GET_MSG = "NEW_GET_MSG";
// 처음 방에 입장할때 이전 대화기록 DB에서 가져오기 (array)
const NEW_SET_MSG = "NEW_SET_MSG";
// 채팅 참여 user 정보
const NEW_GET_CHAT_USER = "NEW_GET_CHAT_USER";
/* ************ 새로 세팅 ************ */ 

/* ************ 새로 세팅 ActionCreator ************ */ 
//채팅방 목록 불러오기
const setNewChatList = createAction(SET_NEW_CHAT_LIST, (myChatList) => ({
  myChatList,
}));
// 현재 채팅방 정보
const moveNewChatRoom = createAction(
  MOVE_NEW_CHAT_ROOM,
...
// 현재 채팅방 정보 초기화
const clearNewChat = createAction(NEW_CLEAR_CHAT, () => {});
// 채팅방 메세지
const getNewMessages = createAction(NEW_GET_MSG, (newMessage) => ({
  newMessage,
}));
// 이전 대화 목록 가져오기
const setNewMessage = createAction(NEW_SET_MSG, (chatMassageArray) => ({
  chatMassageArray,
}));
// 채팅 참여중인 사용자 목록 조회
const getNewChatUser = createAction(NEW_GET_CHAT_USER, (user_in_chat_list) => ({
  user_in_chat_list,
}));
/* ************ 새로 세팅 ActionCreator ************ */ 
...
const setNewChatListAX = () => {
  ...
  let Token = getToken("Authorization");
    ...
    .then((res) => {
      // console.log("새로세팅", res.data);
      dispatch(setNewChatList(res.data))
...
export default handleActions(
  {
    ...
    // 관리자용 새로 세팅잡기 
    // setChatList - 관리자 채팅 목록
    [SET_NEW_CHAT_LIST]: (state, action) =>
      produce(state, (draft) => {
        draft.chatListInfo = action.payload.myChatList;
      }),
  },
